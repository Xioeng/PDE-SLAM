...
class TestStep:
    ...
    def test_east_advances_x_only(self) -> None:
        ...
        state = robot.step(thrust=100.0, compass_heading_rad=np.pi / 2, dt=3.0)
    ...
    def test_north_advances_y_only(self) -> None:
        ...
        state = robot.step(thrust=100.0, compass_heading_rad=0.0, dt=2.0)
    ...
    def test_west_decreases_x(self) -> None:
        ...
        state = robot.step(thrust=100.0, compass_heading_rad=3 * np.pi / 2, dt=1.0)
    ...
    def test_south_decreases_y(self) -> None:
        ...
        state = robot.step(thrust=100.0, compass_heading_rad=np.pi, dt=1.0)
        assert state[0] == pytest.approx(0.0, abs=1e-12)
...
class TestStateAccessors:
    ...
    def test_heading_stored_after_step(self) -> None:
        ...
        heading = np.pi / 4
        state = robot.step(thrust=50.0, compass_heading_rad=heading, dt=1.0)
    ...
    def test_k_thrust_doubles_displacement(self) -> None:
        ...
        s1 = robot1.step(thrust=50.0, compass_heading_rad=np.pi / 2, dt=2.0)
        s2 = robot2.step(thrust=50.0, compass_heading_rad=np.pi / 2, dt=2.0)
...
class TestReset:
    ...
    def test_restores_state(self) -> None:
        ...
        robot = UnicycleKinematics(k_thrust=1.0)
        robot.step(thrust=100.0, compass_heading_rad=np.pi / 2, dt=10.0)
...
class TestTrajectory:
    ...
    def test_shape_with_initial(self) -> None:
        ...
        robot = UnicycleKinematics(k_thrust=1.0)
        n = 10
        states = robot.trajectory(np.ones(n) * 50.0, np.zeros(n), dt=1.0)
    ...
    def test_shape_without_initial(self) -> None:
        ...
        robot = UnicycleKinematics(k_thrust=1.0)
        n = 5
        states = robot.trajectory(np.ones(n) * 100.0, np.zeros(n), dt=1.0, include_initial=False)
    ...
    def test_matches_repeated_step(self) -> None:
        ...
        thrusts = np.array([50.0, 80.0, 30.0])
    ...
    def test_updates_internal_state(self) -> None:
        ...
        robot = UnicycleKinematics(k_thrust=1.0, x0=0.0, y0=0.0)
        n = 5
        states = robot.trajectory(np.ones(n) * 100.0, np.full(n, np.pi / 2), dt=1.0)
...
class TestWaypointFollowing:
    ...
    def test_single_waypoint_reached(self) -> None:
        ...
        # Driving at 2.0 m/s (thrust_cmd = 100.0) with dt=1.0 s, acceptance_radius=5.0 m.
